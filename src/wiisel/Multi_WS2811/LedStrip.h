...
public:
    ...
    static void uncompress(uint16_t cmp_clr, uint8_t* r, uint8_t* g, uint8_t* b);
    static uint16_t compress(uint8_t r, uint8_t g, uint8_t b);

    uint16_t numPixels(void) { return numLEDs; }
    uint16_t numPixelBytes(void) { return numLEDs * 2; }
...
protected:
    uint16_t *pixels;     // Holds LED color values
